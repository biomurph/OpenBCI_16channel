...
/*
    ...
    Place the containing folder into your libraries folder insdie the mpide folder in your Documents folder
...
void OpenBCI_32_Daisy::startStreaming(){  // needs daisy functionality
  if(useAccel){enable_accel(RATE_50HZ);}  // check Definitions.h for other data rates
  startADS(); 
} 

void OpenBCI_32_Daisy::sendChannelData(){
  Serial.write(sampleCounter); // 1 byte
...
boolean OpenBCI_32_Daisy::smellDaisy(void){ // check if daisy present
  ...
  if(verbosity){Serial.print("Daisy ID 0x"); Serial.println(setting,HEX);}
...
void OpenBCI_32_Daisy::removeDaisy(void){
  if(daisyPresent){
    ...
    STANDBY(DAISY_ADS);
    daisyPresent = false;
    if(!isRunning) Serial.println("daisy removed");
  }else{
    if(!isRunning) Serial.println("no daisy to remove!");
  }
}
...
void OpenBCI_32_Daisy::attachDaisy(void){
  ...
  if(!daisyPresent){
    WREG(CONFIG1,0x96,BOARD_ADS); // turn off clk output if no daisy present
    numChannels = 8;    // expect up to 8 ADS channels
    if(!isRunning) Serial.println("no daisy to attach!");
  }else{
    numChannels = 16;   // expect up to 16 ADS channels
    if(!isRunning) Serial.println("daisy attached");
  } 
}
...
{ 
  ...
  if(daisyPresent){
    if(sampleCounter % 2 != 0){ //CHECK SAMPLE ODD-EVEN AND SEND THE APPROPRIATE ADS DATA
      for (int i=0; i<24; i++){ 
        Serial.write(meanBoardDataRaw[i]); // send board data on odd samples
      }
    }else{
      for (int i=0; i<24; i++){
        Serial.write(meanDaisyDataRaw[i]); // send daisy data on even samples
      }
    }
  }else{
    for(int i=0; i<24; i++){
...
void OpenBCI_32_Daisy::LIS3DH_writeAxisData(void){
  for(int i=0; i<3; i++){
    ...
    Serial.write(lowByte(axisData[i]));  // axisData is array of type short (16bit)
  }
}
