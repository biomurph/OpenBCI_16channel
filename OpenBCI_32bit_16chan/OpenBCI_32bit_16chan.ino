  ...
  if(is_running){
      ...
      if(writeToSDonly) {
        if(OBCI.sampleCounter%32 == 0){
          Serial.println(OBCI.sampleCounter,HEX); // verbosity
        }
      }  
...
// DAISY MODULE COMMANDS
      ...
      case 'C':  // use 16 channel mode
        ...
        if(OBCI.daisyPresent){
          Serial.print("16"); outputType = OUTPUT_16_CHAN;
        }else{
          Serial.print("8"); outputType = OUTPUT_8_CHAN;
        }
        sendEOT();     
        break;
...
// STREAM DATA AND FILTER COMMANDS
     ...
     case 's':  // stop streaming data
        ...
        break;
      case 'n':
        Serial.println("write data to SD card only");
...
void sendEOT(){
  Serial.print("$$$");  // shake hands with the PC
}
...
void startFromScratch(){
  if(!is_running){
    ...
    delay(500);
    Serial.println("OpenBCI V3 16 channel");
    OBCI.configureLeadOffDetection(LOFF_MAG_6NA, LOFF_FREQ_31p2HZ);  
    Serial.print("On Board ADS1299 Device ID: 0x"); Serial.println(OBCI.ADS_getDeviceID(ON_BOARD),HEX);
    if(OBCI.daisyPresent){  // library will set this in initialize() if daisy present and functional
      Serial.print("On Daisy ADS1299 Device ID: 0x"); Serial.println(OBCI.ADS_getDeviceID(ON_DAISY),HEX);
    }
