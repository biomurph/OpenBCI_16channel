...
boolean setupSDcard(char limit){
  ...
  default: Serial.println("invalid BLOCK count"); return fileIsOpen; break;
  }
    ...
    openfile.remove(root, currentFileName); // if the file is over-writing, let it!
      ...
      Serial.print("createfdContiguous fail"); cardInit = false;
      }//else{Serial.print("got contiguous file...");delay(1);}
      // get the location of the file's blocks
      if (!openfile.contiguousRange(&bgnBlock, &endBlock)) {
      Serial.print("get contiguousRange fail"); cardInit = false;
      }//else{Serial.print("got file range...");delay(1);}
      // grab the Cache
      pCache = (uint8_t*)volume.cacheClear();
      // tell card to setup for multiple block write with pre-erase
      if (!card.erase(bgnBlock, endBlock)){ Serial.println("erase block fail"); cardInit = false;
      }//else{Serial.print("erased...");delay(1);}
      if (!card.writeStart(bgnBlock, BLOCK_COUNT)){ Serial.println("writeStart fail"); cardInit = false;
     ...
     if(fileIsOpen == true){  // send corresponding file name to controlling program
       Serial.print("Corresponding SD file ");Serial.println(currentFileName);sendEOT();
     }  
     return fileIsOpen;
}
...
void writeCache(){
    ...
    OBCI.csLow(SD_SS);  // take spi
    if(!card.writeData(pCache)) {Serial.println("block write fail");}   // write the block
...
//  Serial.print("Corresponding SD file ");Serial.println(currentFileName); 
}
